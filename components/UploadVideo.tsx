...
export const UploadVideo: React.FC<Props> = ({ isOpen, closeModal }) => {
  const [buttonTxt, setButtonTxt] = useState<string>("Upload");
  const [file, setFile] = useState<File | null>(null);
  const { contract, account, updateVideos } = useData();
  const client = create({ url: "https://demo.storj-ipfs.com/api/v0/add" });
  ...
  const uploadVideo = async () => {
    ...
    contract.methods
      .uploadVideo(added.path, description)
      .send({ from: account })
      .then(async () => {
        await updateVideos();
        setFile(null);
          ...
          <div className="min-h-screen px-4 text-center ">
            ...
            >
              <div className="inline-block w-full p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl  max-w-xl">
                ...
                {/* {file && (
                  <div className="mt-2">
                    <video width={'200px'} src={URL.createObjectURL(file)} />
                  </div>
                )} */}
